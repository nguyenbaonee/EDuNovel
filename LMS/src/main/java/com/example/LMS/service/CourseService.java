    ...
    public CourseResponse createCourse(CourseRequest courseRequest, List<MultipartFile> images) throws IOException {
        ...
        try {
            if (images != null && !images.isEmpty()) {
                ...
                // Fetch thumbnails for response
                List<Image> thumbnails = imageRepo.findByObjectIdAndObjectTypeAndTypeAndStatus(course.getId(), ObjectType.COURSE, ImageType.THUMBNAIL, Status.ACTIVE);
                course.setThumbnail(thumbnails);
                return courseMapper.toCourseResponse(course);
            }
        } catch (Exception e) {
