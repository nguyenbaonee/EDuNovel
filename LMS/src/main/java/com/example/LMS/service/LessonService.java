...
public class LessonService {
    ...
    public LessonResponse createLesson(
        ...
        try {
            ...
            // Fetch videos and thumbnails for response
            List<Image> video2s = imageRepo.findByObjectIdAndObjectTypeAndTypeAndStatus(lesson.getId(), ObjectType.LESSON, ImageType.VIDEO, Status.ACTIVE);
            List<Image> thumbnails = imageRepo.findByObjectIdAndObjectTypeAndTypeAndStatus(lesson.getId(), ObjectType.LESSON, ImageType.THUMBNAIL, Status.ACTIVE);
            lesson.setVideoUrl(video2s);
            lesson.setThumbnail(thumbnails);
    ...
    public LessonResponse updateLesson(Long lessonId, LessonRequest lessonRequest, List<MultipartFile> images,
        ...
        try {
            ...
            // Fetch videos and thumbnails for response
            List<Image> videosFetched = imageRepo.findByObjectIdAndObjectTypeAndTypeAndStatus(lesson.getId(), ObjectType.LESSON, ImageType.VIDEO, Status.ACTIVE);
            List<Image> thumbnailsFetched = imageRepo.findByObjectIdAndObjectTypeAndTypeAndStatus(lesson.getId(), ObjectType.LESSON, ImageType.THUMBNAIL, Status.ACTIVE);
            lesson.setVideoUrl(videosFetched);
            lesson.setThumbnail(thumbnailsFetched);
