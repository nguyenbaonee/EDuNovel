...
public class EnrollmentController {
    ...
    @GetMapping("/{courseId}")
    public Page<Long> getUserIdsOfCourse(@RequestParam(defaultValue = "0") int page,
                                         @RequestParam(defaultValue = "5") int size,
                                         @PathVariable Long courseId,
                                         @RequestParam(defaultValue = "ACTIVE") Status status) {
        return enrollmentService.getUserIdsOfCourse(page, size, courseId, status);
    }
    @GetMapping("/user/{userId}")
    public Page<EnrollmentResponse> getEnrollmentsOfUser(@RequestParam (defaultValue = "0") int page,
                                                         @RequestParam (defaultValue = "5") int size,
                                                         @PathVariable Long userId,
                                                         @RequestParam (defaultValue = "ACTIVE") Status status) {
        return enrollmentService.getEnrollmentsByUser(page, size, userId, status);
    }
    @DeleteMapping("/{id}")
